...
build:
	...
	@GOOS=windows GOARCH=amd64 go build -ldflags="-X main.version=$(VERSION)" -o build/plt-windows-amd64.exe plt.go \
		&& zip -9 -D build/plt-windows-amd64.zip build/plt-windows-amd64.exe && rm build/plt-windows-amd64.exe
	@echo ">> building binaries - alpine_amd64"
	@docker run --rm -v $(PWD):/app -w /app golang:1.15-alpine go build -ldflags="-X main.version=$(VERSION)" -o build/plt-alpine-amd64 \
